...
impl AuditLog {
    ...
    pub fn write(
        ...
        let snap_escaped = serde_json::to_string(&record.snapshot_path.as_ref().map(|p| p.to_string_lossy().to_string()).unwrap_or_default()).unwrap_or_default();
...
impl AuditLogParser {
    ...
    fn parse_line(line: &str) -> Option<ChangeRecord> {
        ...
        if !rest.starts_with("audit_v1 ") { return None; }
            ...
            if rest.starts_with('"') {
                ...
                match key {
                    ...
                    "bbox" => {
                        ...
                        let parts: Vec<f32> = clean.split(',').filter_map(|p| p.parse().ok()).collect();
                        if parts.len() == 4 {
...
mod tests {
    ...
    fn roundtrip_records() {
        ...
        audit.write(&rec1, Path::new("in"), Path::new("out"), "test", false).unwrap();
        ...
        assert_eq!(parsed[0].snapshot_path, Some(PathBuf::from("audit/snapshots/123.pdf")));
    }
