...
impl GeometryProvider for PyMuPdfHeuristicProvider {
    fn extract_line_geometry(&self, pdf_path: &Path) -> Result<Vec<LineGeometry>, ExtractorError> {
        ...
        let date_pattern = Regex::new(r"(?i)(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|\d{1,2}/\d{1,2})").unwrap();
            ...
            for block in blocks {
                ...
                if current_y.is_none() {
                    ...
                    if (y_center - y).abs() < 5.0 {
                        ...
                        // Process row
                        process_row(&current_row, page, row_idx, &mut geometries, &date_pattern, &amount_pattern);
                        current_row.clear();
                        current_row.push(block);
                        current_y = Some(y_center);
                        row_idx += 1;
