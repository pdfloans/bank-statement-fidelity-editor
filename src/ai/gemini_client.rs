        ...
        match self.post_generate_pro(&body).await {
            ...
            err => {
                ...
                for tx in transactions {
                    sum_credits += tx.credit.map(|d| d.to_string().parse::<f64>().unwrap_or(0.0)).unwrap_or(0.0);
                    sum_debits += tx.debit.map(|d| d.to_string().parse::<f64>().unwrap_or(0.0)).unwrap_or(0.0);
                }
