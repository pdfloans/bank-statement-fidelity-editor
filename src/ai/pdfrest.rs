...
impl std::fmt::Debug for PdfRestClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PdfRestClient")
            .field("api_key", &format!("<masked: {} chars>", self.api_key.len()))
...
impl PdfRestClient {
    ...
    pub async fn render_pdf_to_images(&self, pdf: &Path, out_dir: &Path, dpi: u32) -> Result<Vec<PathBuf>, PdfRestError> {
        fs::create_dir_all(out_dir).await?;

        let pdf_bytes = fs::read(pdf).await?;
        let filename = pdf.file_name().and_then(|n| n.to_str()).unwrap_or("document.pdf").to_string();

        let form = multipart::Form::new()
            .part("file", multipart::Part::bytes(pdf_bytes)
                .file_name(filename)
        ...
        let upload_res: UploadResponse = resp.json().await.map_err(|e| PdfRestError::BadResponse(e.to_string()))?;
        ...
        if let Some(url) = upload_res.output_url {
            ...
            loop {
                ...
                let poll_resp = self.authed_request(reqwest::Method::GET, &poll_url)
                    .send()
                    .await
                    .map_err(|e| PdfRestError::Poll(e.to_string()))?;
                
                if poll_resp.status().is_success() {
                    let res: ResourceResponse = poll_resp.json().await.map_err(|e| PdfRestError::Poll(e.to_string()))?;
            ...
            return Err(PdfRestError::BadResponse("No outputUrl or outputId in response".into()));
        }

        let mut downloaded_paths = Vec::new();
        for (i, url) in output_urls.into_iter().enumerate() {
            let download_resp = self.authed_request(reqwest::Method::GET, &url)
                .send()
                .await
                .map_err(|e| PdfRestError::Download(e.to_string()))?;

            if !download_resp.status().is_success() {
                return Err(PdfRestError::Download(format!("Status: {}", download_resp.status())));
            }

            let bytes = download_resp.bytes().await.map_err(|e| PdfRestError::Download(e.to_string()))?;
...
mod tests {
    ...
    async fn happy_path_uploads_and_downloads() {
        ...
        let temp_dir = std::env::temp_dir().join("pdfrest_test_happy");
        let pdf_path = temp_dir.join("test.pdf");
        fs::create_dir_all(&temp_dir).await.unwrap();
        fs::write(&pdf_path, b"%PDF-1.4").await.unwrap();

        let results = client.render_pdf_to_images(&pdf_path, &temp_dir, 300).await.unwrap();
    ...
    async fn auth_failure_returns_pdfrest_error_auth() {
        ...
        let temp_dir = std::env::temp_dir().join("pdfrest_test_auth");
        let pdf_path = temp_dir.join("test.pdf");
        fs::create_dir_all(&temp_dir).await.unwrap();
        fs::write(&pdf_path, b"%PDF-1.4").await.unwrap();

        let result = client.render_pdf_to_images(&pdf_path, &temp_dir, 300).await;
    ...
    async fn poll_path_completes_after_running_state() {
        ...
        let result = client.render_pdf_to_images(&pdf_path, out_dir.path(), 300).await.unwrap();
