...
pub fn merge_consensus_statements(statements: Vec<BankStatement>) -> BankStatement {
    ...
    let majority_opening = opening_votes.into_iter().max_by_key(|&(_, v)| v).map(|(k, _)| k).unwrap_or(Decimal::ZERO);
    ...
    let majority_closing = closing_votes.into_iter().max_by_key(|&(_, v)| v).map(|(k, _)| k).unwrap_or(Decimal::ZERO);
