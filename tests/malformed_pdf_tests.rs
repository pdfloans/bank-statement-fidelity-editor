...
async fn test_malformed_pdf_handling() {
    ...
    assert!(res.is_err(), "Engine should cleanly reject a zero-byte PDF instead of panicking");
    
    // 2. Corrupt PDF syntax handling (not really PDF)
    let corrupt_path = PathBuf::from("tests/stress_pdfs/corrupt.pdf");
    fs::write(&corrupt_path, "This is absolutely not a valid PDF file! %PDF-1.4 %%EOF").unwrap();
