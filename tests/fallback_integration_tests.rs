...
use dual_core_pdf_pipeline::pdf::engine::{EngineCapabilities, ReplaceOutcome, RenderedPage, TextBlock};
use dual_core_pdf_pipeline::engine::layout::DocumentLayout;

#[derive(Debug)]
struct DummyEngine;
impl PdfEngine for DummyEngine {
    fn capabilities(&self) -> EngineCapabilities { EngineCapabilities { supports_redaction: false, supports_cjk: false, supports_embedded_fonts: false, estimated_fidelity: 0.0 } }
    fn render_page(&self, _path: &std::path::Path, _page: usize, _dpi: f32) -> Result<RenderedPage, EngineError> { Ok(RenderedPage { png_bytes: vec![], width_pts: 100.0, height_pts: 100.0 }) }
...
async fn test_offline_fallback_can_be_triggered() {
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        dual_core_pdf_pipeline::engine::offline_parser::parse_statement_offline(&path, engine_for_tokio)
    }).await;
    ...
    match result {
        ...
        Ok(Err(e)) => {
            ...
            assert!(e.to_string().contains("Failed to parse statement offline"), "Fallback successfully ran and safely handled the missing file");
        }
        Err(e) => {
