...
async fn test_ai_backend_cascade_success_on_primary() {
    ...
    let mock_gemini = server
        .mock("POST", "/v1beta/models/gemini-pro-latest:generateContent?key=gemini-key")
        .with_status(500)
        .expect(0)
        .create_async().await;
    ...
    backend.openrouter = Some(OpenAiClient::with_base_url("or-key".to_string(), server.url(), "or-model".to_string()));
    backend.gemini = Some(GeminiClient::with_base_url("gemini-key".to_string(), server.url()));
    backend.groq = Some(OpenAiClient::with_base_url("groq-key".to_string(), server.url(), "groq-model".to_string()));
